...
calcBmi.addEventListener("click",async function() {
  ...
  if (bmicalc < 16 ) {
     Status = "Under Weight";
     parag =  "u need to gain more weight X club can make u biger than BIGRAMY"
  } else if (16 <= bmicalc &&  bmicalc <= 25) {
     Status = "Normal Weight";
     parag =  "u look so good but start now with X club to be new Ronnie Coleman"
  }else if (25,1 <= bmicalc &&  bmicalc <= 30){
    Status = "Over Weight";
    parag =  "u need to be in X club to lose some fats and gain Alot muscles"
  }else if (30.1 <= bmicalc &&  bmicalc <= 35){
    Status = "Obses II Weight";
    parag =  "why u still in your home X Club is your new home"
  }else if (35.1 <= bmicalc &&  bmicalc <= 40){
    Status = "Obses III Weight";
    parag =  "X club here to Ignite your fats"
  }else{
   Status = "Too Much";
    parag =  "X club can't support this body"
  }
  clearFields()
...
function resultBMI(bmicalc,Status,parag) {
 let cartona = `                <div class="col-md-6">
                ...
                <h5 class="section-title">${parag}</h5>
                </div>
                </div>`
